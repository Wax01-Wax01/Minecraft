...
while i < 999:
    ...
    while k < game_size:
        ...
        k += 1
    print(f'Inventory: {Grass} grass, {Wood} wood, {Lotteries} lotteries, {Leaves} leaves, {Saplings} saplings, {Stone} stone, {Planks} planks')
    ...
    if (move.upper() == 'JUMP' or move.upper() == ' ' or move.upper() == 'W') and touching_ground is True:
        up_speed = 1
        last_move = 'jump'
    elif (move.upper() == 'LEFT' or move.upper() == 'A') and place % game_size != 0 and game[place - 1] != '▪' and game[place - 1] != '|' and game[place - 1] != '0' and game[place - 1] != '◈' and game[place - 1] != '∥':
        game[place % (game_size**2)] = ' '
        place -= 1
        game[place % (game_size**2)] = '🙂'
    elif (move.upper() == 'RIGHT' or move.upper() == 'D') and place % game_size != game_size - 1 and game[place + 1] != '▪' and game[place + 1] != '|' and game[place + 1] != '0' and game[place + 1] != '◈' and game[place + 1] != '∥':
        game[place % (game_size**2)] = ' '
        place += 1
        game[place % (game_size**2)] = '🙂'
        ...
        try:
            ...
            if ((int(x_pos) - int(x)) ** 2 + (int(y_pos) - int(y)) ** 2) ** 0.5 < 2.9 and (game[place_break] == '▪' or game[place_break] == '|' or game[place_break] == '0' or game[place_break] == '◈' or game[place_break] == '∥'):
                if game[place_break] == '▪':
                    Grass += 1
                    game[place_break] = ' '
                if game[place_break] == '|':
        ...
        try:
            ...
            if ((int(x_pos) - int(x)) ** 2 + (int(y_pos) - int(y)) ** 2) ** 0.5 < 2.9 and game[place_break] == ' ':
                ...
                if block.upper() == 'STONE' and Stone > 0:
                    game[place_break] = '◈'
                    Stone -= 1
                if block.upper() == 'PLANKS' and Planks > 0:
                    game[place_break] = '∥'
                    Planks -= 1
        except ValueError:
            pass
        ...
        if craft == '1':
            ...
            if craft_count.isdigit():
                if Wood >= 1 * int(craft_count):
                    Wood -= 1 * int(craft_count)
                    Planks += 4 * int(craft_count)
    game[place % (game_size**2)] = ' '
    if up_speed > 0:
        i = 0
        while i < up_speed:
